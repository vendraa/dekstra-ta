...
import { downloadBerkas } from "@/features/requests/services/berkas.service";
...
function ActionCell({ request }: { request: LetterRequest }) {
  ...
  const isApproved = request.lifecycle === "COMPLETED";
  ...
  return (
    <>
      <div className="flex justify-center gap-2">
        ...
        {/* 🔥 Download hanya jika disetujui */}
        {isApproved && request.publicToken && (
          <Button
            size="sm"
            className="bg-secondary text-white"
            onClick={() => downloadBerkas(request.publicToken!)}
          >
            <Download size={14} />
            Unduh
          </Button>
...
export const riwayatRequestColumns: ColumnDef<LetterRequest>[] = [
  {
